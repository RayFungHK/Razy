...
`RAZY_VERSION` verified from the archive itself; rebuilt again at the tag-bearing commit so
the phar matches its tree byte-for-byte (the corrections below changed `src/`).

## Pre-tag corrections (folded into this release)

Three commits landed between the release commit and the tag — each found by USING the
release, and each small enough to ride along rather than defer:

- **`markdown_service`'s API could never resolve** (`746d84a`). Double bug: closures lived
  outside `controller/` (where `ClosureLoader` looks) AND the registrations carried `.php`
  while the loader appends it (`parse.php.php`). Proven pre-existing by differential against
  the previous phar; the release ritual's own `validate` step surfaced it. The fix also
  decontaminated the three TEACHING surfaces that spread the form: the README example,
  the `generate-skills` module template (an AI agent copies it verbatim — worst vector),
  and api_demo's HTML snippet. `demos/README.md:92` had documented the correct rule all
  along; the violator ignored its own repo's doc.
- **`generate-skills` produced zero module skills, silently** (`b822edb`). The command
  existed since well before this release and promised `skills/{dist}/{module}.md` for
  every module; a live run post-release found all 30 modules erroring and ZERO module
  files ever written, while the runner printed per-module green ✓. Five independent
  silent deaths fixed: undefined `Module::getPath()` (it is `ModuleInfo::getPath()`);
  API section probing a fictional `getAPI()` (real door: `getAPICommands()`); events
  section probing a fictional `getListeners()` (new read-only door
  `Module::getEventRegistrations()` — raw table, closure listeners included, unlike the
  compile door's filtered dump); `@llm` scanning rooted at a `src/` folder no module
  layout has (now tree-wide docblock regex with honest line numbers); and the deepest —
  module codes carry a slash, so the output "filename" nested directories a flat `mkdir`
  never made and `file_put_contents` failed SILENTLY (writes now mkdir deep and throw;
  a ✓ must be a receipt). Verified live: 32 skill files, events and `@llm` prompts
  present. The method_exists guesses were the same disease AGENTS.md names for emitters
  (RZ-017's lineage): guess instead of consult the door.
- **README badges refreshed** (`b822edb`): version shield `1.2.0`, tests shield 5,565 —
  honest as of this release, the shields had drifted two majors behind.
