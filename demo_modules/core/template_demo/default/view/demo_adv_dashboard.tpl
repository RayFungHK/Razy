<h3>{$dashboard_title->escape}</h3>
<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px">
<!-- START BLOCK: card -->
<div style="padding:16px;border:1px solid #e0e0e0;border-radius:8px;background:#fafafa">
    <div style="font-size:24px;margin-bottom:4px">{$icon->escape}</div>
    <div style="font-size:13px;color:#888;text-transform:uppercase">{$title->escape}</div>
    <div style="font-size:28px;font-weight:bold;margin:4px 0">{$value->escape}</div>
