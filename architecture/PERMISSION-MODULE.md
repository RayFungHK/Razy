...
| `audit-actor(string $actorKey): array` *(S5 — S4 fixed this row: §8 audit is an EVENT, a "last denials" read side would require the audit table §8 deliberately does not ship; deferred as the S5 row itself already said)* | last denials for one actor, read side | backs Q-audit surface |
