...
| `audit-actor(string $actorKey): array` *(S5 — ✅ shipped exactly as drafted, opt-in table via the second migration; S4 had fixed this row from *(S4)*: the dossier contradicted itself, §8's "audit is an event" won until the table arrived)* | last denials for one actor, read side | backs Q-audit surface |
...
public function __onAPICall(ModuleInfo $module, string $method, string $fqdn = ''): bool {
    ...
    if (isset(self::GOVERN_ONLY[$method]))
        return $module->getCode() === ($this->getModuleConfig()['governor'] ?? ''); // writes: named only — ArrayAccess, NOT ->get() (see ledger P8; shipped code additionally rejects empty-string governors)
...
| **S2 `razymod/permissions` skeleton + schema** — ✅ **SHIPPED 2026-09** (migration is driver-branched raw DDL per the `DatabaseStore::ensureStorage` precedent so up/down runs END-TO-END on sqlite — Table's MySQL-only getSyntax made the milestone's wording impossible without it; Contract mirror parity-checked against the LIVE schema via PRAGMA; gate matrix 27 cases incl. empty-governor edge; ledger P8 found+fixed en route) | module dir (`module.php` like `queue-admin/module.php:12-17`), `migration/` tables §7.1 (Column grammar), `ORM\Contract` mirror, config keys (`super_actors`, `governor`, default roles), DB via config-connect (§4.2 option 1), `__onAPICall` two-tier allow-list (§8) | `modules/permissions/{module.php, default/{package.php, migration/*, controller/permissions.php, controller/api/*}}` | **M** · migration up/down on sqlite + mysql fixtures (`tests/MigrationTest.php` pattern `:1291`); allow-list matrix test per RZ-014 (every command × allowed/denied caller) |
| **S3 Check surface + registration** — ✅ **SHIPPED 2026-09** (deviations, stated: (a) Gate builds LAZILY on first `gate()` call, NOT at boot — boot-time DB IO per request contradicts RZ-009 cost doctrine (reaffirmed by MIGRATION-GOVERNANCE.md E5 the same week); (b) catalog codes are NOT `Gate::define`d at boot — the before-hook is DECISIVE (always bool), the DB is the single policy source, `abilities()` reads the catalog directly; (c) controller class is NAMED (`support/controller.php` returns-instance still) so api handlers can annotate their `$this` — phpstan cannot rebind `$this` from standalone closure files (varTag.variableNotFound by design), so `phpstan.modules.neon` analyses everything EXCEPT `controller/api` (limitation documented; every handler is executed by the 16 new tests instead — RZ-014 coverage is by tests); (d) no cross-request memo by construction: one Service per call; S5 may add Cache-layer memo. Phantoms/quirks caught en route: raw `prepare($sql)->assign()` does NOT bind (assign feeds ORM syntax builders only, Statement.php:205-217) — all reads moved to the sanctioned builder; `Database::insert(table, **column-list**)->assign(values)` (assoc-as-columns silently drops values through the identifier-regex filter, Statement.php:575-582); `Query::affected()` not `rowCount()`; `Statement::delete` carries params+where in its own signature) | `can/can-any/abilities/define-ability` open + `roles-of/assign-role/revoke-role` governor-only (S2 gate now fronts REAL handlers, 1:1 with the pinned matrix); decision grammar guest>super>db incl. env-extend, CLI system_actors, session actor key; idempotent mutations with write-time memo invalidation; `permission.denied` fires on gated db-denials only | `default/controller/{permissions.php, support/{controller,Service}.php, api/*}`, `phpstan.modules.neon` | **M** · `tests/PermissionsCheckSurfaceTest.php` (16): default-deny, guest-pre-super (§7.3 pinned), env EXTENDS config, CLI posture web-never, never-throw on dead DB, memo no-refetch + write-invalidate, assign/revoke idempotence + unknown-role refusal, dotted grammar, Gate composition (decisive hook, guest pre-deny untouched) — 51 permission tests green |
| **S4 Templates + audit + docs** — ✅ **SHIPPED 2026-09** (deviations, stated: (a) `audit-actor` DEFERRED to S5 — this dossier contradicted itself (the :298 row said S4, the S5 row deferred it); §8 "audit is an event, not a table" wins, a read side needs a table §8 refuses to ship; row fixed in place; (b) tag syntax `{@can …}{/can}` — `@`-prefix per manual/05 §3.4 (the S4 planning wording `{can}` was drift); (c) golden demo = NEW `golden/policy` consumer demo (provider/consumer pair untouched); (d) the S4 template tests drove `controller->service()` for the FIRST time and caught a latent S3 bug — `require __DIR__ . '/support/database.php'` inside `support/` resolved to `support/support/` (S3 tests built Service directly, never through the controller: the tier the check-surface suite lacked, now paid for itself); (e) PluginManager memoizes plugin instances per folder (PluginManager.php:109 — GateFactory lesson again); tests reset via the documented `Template::resetPlugins()` worker-mode antidote) | `default/plugins/Template/function.can.php` (TFunctionCustom, ml shape, instanceof-narrowed binder, never-throw, deny = markup never ships), `registerPluginLoader(PLUGIN_TEMPLATE)` at `__onInit`, `canAbilities()` session-bound check, `manual/11-permissions.md` (+README row, 09/10 numbering explained), `demos/golden/policy/*` (fail-closed consumer, `responseCode(403)` verified), razit→permissions migration table (manual/11 §9) | **M** · `tests/PermissionsTemplateTest.php` (8): real-engine render through the module's own registration path — guest deny-never-ships, super, env-extend, db-grant/absence, OR-list, dead-db-hides-not-breaks, RZ-004 pass-through/never-ships pair, path pin |

<!-- ORIGINAL S4 ROW (kept for the record): -->
...
| **S5 Governance polish** — ✅ **SHIPPED 2026-09** (the user demanded it; the gates then decided individually: **cache layer ✅** opt-in `cache_ttl` (default 0 = the shipped posture is STILL zero cross-request state), `Razy\Cache\CacheInterface` injected, §7.6's MANDATED invalidation-on-write always armed — a no-op `assign-role` deliberately does NOT invalidate (nothing went stale: invalidating on no-op erases the cache's purpose, pinned by test); foreign cache values recompute, never grant; **audit-actor ✅** as the OPT-IN table (`audit` config, second migration, module listens to its own qualified event, best-effort inserts, governor-only read, newest-first with limit — event remains primary, default off keeps §8 doctrine intact); **direct grants ❌ stay OFF** — Q5 reopened only by a NAMED USE CASE, and "do S5" names none (recorded honestly rather than shipping schema space on a vague mandate); **admin UI ❌ not shipped** — Q4 decided no mutation HTTP surface and a read shell was never demanded; the queue-admin `/ui` ceiling describes a limit, not a request) | `Service` cache layer + `invalidateActor` + `auditLog`/`auditActor`, controller audit listener + `audit-actor` command (`GOVERN_ONLY` joined), `api/audit_actor.php`, migration `AddPermissionAuditLog` + Contract mirror, versions 0.2.0 (RZ-012 minor), manual/11 updated | **M** · `tests/PermissionsS5Test.php` (8): zero-query cache hit through a fresh Service, invalidate-on-real-write both verbs + no-op-preserves + empty-deny-set caching + `invalidateActor`, default-uncached + ttl-0-with-adapter + adapter-less (fail-SAFE), foreign-shape recompute, listener registered only when on, rows newest-first/limit/empty-key/no-db-silent; S2 suite updated for the 5th table/two-migration batch/audit-actor gate row |

<!-- ORIGINAL S5 ROW (kept for the record): -->
...
- **Q3 — `Database::getSharedInstance()` phantom**: define it (set-once/get by app boot,
  ...
  → **Define-it SHIPPED (§4.4 note); fail-loud rework SHIPPED 2026-09**: the resolver split into two
  named failure classes (shared-instance-absent → config gap named + bootstrap fix; construction
  Throwable → class + message verbatim), `exit(1)` on every unreachable-queue path including retry's
  usage error, dead `class_exists` guard removed — `tests/QueueCommandTest.php` pins it (source-pin
  house style); the §4.4 "tests never hit it" gap is closed.
...
| P8 | `manual/04-database.md:50` advertises `$this->getModuleConfig()->get(...)` | **`Configuration` has no `get()` method** (whole-class read, `Configuration.php`: only offsetSet/save/saveAs*; `Collection extends ArrayObject` `Collection.php:31` → read API is ArrayAccess `['key'] ?? default`) — second phantom-of-the-session, found the moment S2 code believed the manual; corrected at the source 2026-09 alongside razymod/permissions S2 |
