# Razy Framework LLM-CAS

**Type**: Coder Assistant System (LLM Navigation Hub) | **Framework**: Razy v1.0.1-beta | **Updated**: February 27, 2026

---

## What is LLM-CAS?

Meta-documentation system helping LLM agents and developers navigate the Razy framework. Use this to find the right document and understand the architecture.
...
| Generate LLM context docs | `php Razy.phar generate-llm-docs` (auto-generates module/distribution docs) |
...
```
Root Files:
├── LLM-CAS.md            # Framework architecture & rules (this file)
...
src/asset/prompt/          # LLM-CAS auto-generation templates
├── LLM-CAS.md.tpl       # Application-level context template
├── LLM-CAS-MODULE.md.tpl      # Per-module context template
└── LLM-CAS-DISTRIBUTION.md.tpl # Per-distribution context template
```

### LLM-CAS Auto-Generation (`generate-llm-docs`)

The `generate-llm-docs` CLI command auto-generates contextual documentation for LLM agents working on Razy-based projects:

```bash
php Razy.phar generate-llm-docs            # Generate all (framework + distributions + modules)
php Razy.phar generate-llm-docs --root-only # Generate only root LLM-CAS.md
```

**Generated files:**
| File | Purpose |
|------|---------|
| `LLM-CAS.md` | Framework overview for the project |
| `llm-cas/{dist_code}.md` | Distribution context (domains, modules, config) |
| `llm-cas/{dist_code}/{module}.md` | Module context (APIs, events, files, `@llm` prompts) |
...
6. **LLM-CAS.md** - Update Quick Navigation, Demo Modules, and Important Commands tables when relevant features change.
...
| `php Razy.phar generate-llm-docs` | Generate LLM-CAS.md files for framework and modules |
...
5. **Documentation** — Update LLM-CAS.md if architecture changes
...
## Using LLM-CAS in Your Project

**Auto-generation**: Run `php Razy.phar generate-llm-docs` to automatically generate LLM-CAS files for your project, distributions, and modules using the templates in `src/asset/prompt/`.

**Manual**: Copy this as a template for your framework/project:

1. Replace **What is LLM-CAS?** with your project purpose
...
**Adaptable Template**: Use as LLM-CAS for any PHP framework
