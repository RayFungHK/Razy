        ...
        if ($showModules) {
            ...
            try {
                ...
                if (empty($modules)) {
                    ...
                    $this->writeLineLogging('  ┌─────┬──────────────────────────────┬─────────┬──────────────┬──────────────────┐', true);
                    $this->writeLineLogging(sprintf('  │ {@c:green}%-2s{@reset}  │ {@c:green}%-28s{@reset} │ {@c:green}%-7s{@reset} │ {@c:green}%-12s{@reset} │ {@c:green}%-16s{@reset} │', 
                        '#', 'Module Code', 'Version', 'Status', 'Type'), true);
                    $this->writeLineLogging('  ├─────┼──────────────────────────────┼─────────┼──────────────┼──────────────────┤', true);
                    ...
                    foreach ($modules as $moduleCode => $module) {
                        ...
                        $this->writeLineLogging(sprintf('  │ %2d  │ %-28s │ %-7s │ %s%-12s{@reset} │ %s%-16s{@reset} │', 
                            $moduleIndex, 
                            $moduleCode, 
                            $moduleVersion,
                            $statusColor,
