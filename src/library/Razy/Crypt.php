...
class Crypt
{
	 ...
	 * @return string
	 */
	static public function Encrypt(string $text, string $key, bool $toHex = false): string
	{
	 ...
	 * @return string
	 */
	static public function Decrypt(string $encryptedText, string $key): string
	{
		// If the encrypted text is hex string, convert to binary
		if (preg_match('/^[a-z0-9]+$/', $encryptedText)) {
